...
if [ "$BACKUP_TYPE" = "initial" ]; then
	...
	backup_log "❌    Invalid backup type: ${BACKUP_TYPE}"
	exit 1
fi
...
if [ -s "$CREATE_ALL_BACKUPS_FILE" ]; then
	backup_log "(${BACKUP_TYPE})    ✅    ${C_Cyan}Full Backup success for: ${C_BrightGreen}$DB_HOST_NAME${C_Reset}"
	backup_log "(${BACKUP_TYPE})    📦    -> ${C_BrightGreen}$CREATE_ALL_BACKUPS_FILE${C_Reset}"
else
	backup_log "(${BACKUP_TYPE})    ❌    ${C_BrightYellow}Full Backup failed for: ${C_Red}$DB_HOST_NAME${C_Reset}"
	backup_log "(${BACKUP_TYPE})    📦    -> ${C_Red}$CREATE_ALL_BACKUPS_FILE${C_Reset}"
...
for DB in $DATABASES; do
	...
	if [ -s "$DB_BACKUPS_FILE" ]; then
		backup_log "(${BACKUP_TYPE}) ✅    ${C_Cyan}Backup success for: ${C_BrightGreen}$DB${C_Reset}"
		backup_log "(${BACKUP_TYPE}) 📦    -> ${C_BrightGreen}$DB_BACKUPS_FILE${C_Reset}"
	else
		backup_log "(${BACKUP_TYPE}) ❌    ${C_BrightYellow}Backup failed for: ${C_Red}$DB${C_Reset}"
		backup_log "(${BACKUP_TYPE}) 📦    -> ${C_Red}$DB_BACKUPS_FILE${C_Reset}"
...
if [ -f "$BACKUPS_CLEANUP_SCRIPT_FILE" ]; then
	backup_log "(${BACKUP_TYPE})    🧹    Running cleanup: $BACKUPS_CLEANUP_SCRIPT_DESC"
