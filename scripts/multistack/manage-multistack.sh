...
case "$selected_option" in
    ...
    *)
        ...
        if printf "%s" "$selected_option" | grep -q "^Manage "; then
            ...
            wp_options="$wp_options|Start ${STACK_NAME}|Restart ${STACK_NAME}|Stop ${STACK_NAME}|Open https://${LEMP_SERVER_DOMAIN} in Browser|Open https://phpmyadmin.${LEMP_SERVER_DOMAIN} in Browser|Database: Backup Dump|Database: Recover Tables|Remove ${STACK_NAME}|${STACK_NAME} Info|Help"
            ...
            STACK_NAME_UPPER=$(printf "%s" "$STACK_NAME" | tr '[:lower:]' '[:upper:]')
            section_title "$STACK_NAME_UPPER STACK OPTIONS" ${C_Magenta}
            i=1
            OLD_IFS=$IFS; IFS='|' # Set delimiter for correct iteration
            for option in $wp_options; do
                ...
                if [ -n "$containers" ] && [ "$container_start_index" -gt 0 ] && [ "$i" -eq "$container_start_index" ]; then
                    line_break
                    section_title "${STACK_NAME_UPPER} CONTAINERS" ${C_Magenta}
                fi
                option_msg "$i. $option" ${C_Magenta}
                i=$((i + 1))
            done
