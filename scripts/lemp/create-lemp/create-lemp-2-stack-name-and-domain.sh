...
example_msg "EXAMPLE"
example_msg
example_msg "This example is using a ${C_Status}random word${C_Reset} generator to create a unique LEMP stack name and server domain name."
example_msg
example_msg "Random word: ${C_Status}${C_Underline}${LEMP_SERVER_DOMAIN_NAME_DEFAULT}${C_Reset}"
example_msg
example_msg "${C_Status}${C_Underline}\$LEMP_DIR: ${C_Reset}Names the directory in which the LEMP Stack files are located"
example_msg
example_msg "- ${STACKS_PATH}/${C_Status}${C_Underline}${LEMP_SERVER_DOMAIN_NAME_DEFAULT}${C_Reset}"
example_msg
example_msg "${C_Status}${C_Underline}\$LEMP_SERVER_DOMAIN_NAME: ${C_Reset}Names the local development domain (separate from the nested WordPress domains)"
example_msg
example_msg "- https://${C_Status}${C_Underline}${LEMP_SERVER_DOMAIN_NAME_DEFAULT}${C_Reset}.${LEMP_SERVER_DOMAIN_TLD_DEFAULT}"
line_break
...
warning_msg "leave blank to use the random word: ${C_Status}${C_Underline}${LEMP_SERVER_DOMAIN_NAME_DEFAULT}${C_Reset}"
line_break
...
if [ -z "$USER_INPUT_LEMP_SERVER_DOMAIN_NAME" ]; then
    ...
    export LEMP_SERVER_DOMAIN_NAME="$(sanitize_string "${USER_INPUT_LEMP_SERVER_DOMAIN_NAME}" "-")"
fi
...
example_msg "${C_Status}EXAMPLE"
example_msg
example_msg "${C_Status}This example is using the input above${C_Reset}"
example_msg
example_msg "${C_Status}\$LEMP_SERVER_DOMAIN_TLD: ${C_Reset}Names the local development domain's top-level domain (TLD)"
example_msg
example_msg "- https://${LEMP_SERVER_DOMAIN_NAME}.${C_Status}${C_Underline}${LEMP_SERVER_DOMAIN_TLD_DEFAULT}${C_Reset}"
example_msg
line_break
...
export PHPMYADMIN_SUBDOMAIN="phpmyadmin.${LEMP_SERVER_DOMAIN}"
#####################################################
# CREATE LEMP STACK
