...
heading "ADD SUBDOMAIN TO HOSTS FILE"
...
if [ -z "$OS_NAME" ] || [ -z "$WORDPRESS_SUBDOMAIN" ] || [ -z "$HOSTS_FILE_LOOPBACK_IP" ] || \
   [ -z "$LEMP_SERVER_DOMAIN" ] || [ -z "$LEMP_SERVER_DOMAIN_TLD" ] || [ -z "$HOSTS_FILE" ]; then
	line_break
fi
