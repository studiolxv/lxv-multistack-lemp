...
start_docker_desktop() {
    ...
    case "$os" in
        Darwin)
            ...
            if ! macos_docker_proc_running; then
                body_msg "🔎 Detected macOS"
                sudo open -a "Docker Desktop" || warning_msg "⚠️ Failed to invoke Docker.app (check permissions/TCC) or try opening it manually."
            fi
        ;;
        Linux)
...
wait_for_docker_daemon_ready() {
    ...
    while :; do
        ...
        status_msg "Waiting for Docker daemon to be ready... (${elapsed}s/${timeout}s)"
        sleep "$interval"
